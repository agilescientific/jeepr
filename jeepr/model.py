...
class Model(np.ndarray):
    ...
    # @property
    # def velocity(self):
    #     material_perms = utils.get_materials(infile)
    #     material_list = [i[0] for i in sorted(mapping.items(), key=operator.itemgetter(1)) if '+' not in i[0]]
    #     perms = np.array([material_perms[i] for i in material_list])
    #     earth_model = perms[mat].squeeze()
    #     return utils.c / np.sqrt(m)
